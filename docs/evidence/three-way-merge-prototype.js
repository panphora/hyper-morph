...
function merge3Text(base, local, remote) {
  ...
  const L = charHunks(base, local),
    R = charHunks(base, remote);
  let out = "",
    pos = 0,
    li = 0,
    ri = 0,
  ...
  while (li < L.length || ri < R.length) {
    ...
    if (other && other.bs < h.be && h.bs < other.be) {
      ...
      out += base.slice(pos, rh.bs) + rh.text;
      pos = rh.be;
      conflict = true;
      li++;
      ri++;
...
const isEl = (n) => n.nodeType === 1,
  isText = (n) => n.nodeType === 3;
const sig = (el) =>
  el.tagName +
  "|" +
  [...el.classList].sort().join(" ") +
  "|" +
  ["href", "src", "name", "type", "role"]
    .map((a) => el.getAttribute(a) || "")
    .join("|");
const hint = (n) =>
  (n.textContent || "").replace(/\s+/g, " ").trim().slice(0, 64);
const exactKey = (n) =>
  isEl(n) ? "E" + n.outerHTML : (isText(n) ? "T" : "C") + n.nodeValue;
...
const similarEl = (a, b) => {
  ...
  if (!x && !y) return true;
  if (!x || !y) return false;
  const h = charHunks(x, y);
  const changed = h.reduce((n, k) => n + (k.be - k.bs) + k.text.length, 0);
...
function uniquePairs(bs, ss, keyOf, map, used) {
  ...
  for (const b of bs) {
    const k = keyOf(b);
    if (k != null) cb.set(k, (cb.get(k) || 0) + 1);
  }
  for (const s of ss) {
    const k = keyOf(s);
    if (k != null) cs.set(k, (cs.get(k) || 0) + 1);
  }
  for (const b of bs) {
    const k = keyOf(b);
    if (k != null && cb.get(k) === 1 && cs.get(k) === 1) {
      const s = ss.find((x) => keyOf(x) === k);
      map.set(b, s);
      used.add(s);
    }
  }
...
function alignKids(bKids, sKids, map, used) {
  const rem = () => [
    bKids.filter((b) => !map.has(b)),
    sKids.filter((s) => !used.has(s)),
  ];
  let [rb, rs] = rem();
  uniquePairs(rb, rs, exactKey, map, used);
  [rb, rs] = rem();
  uniquePairs(
    rb,
    rs,
    (n) => (isEl(n) ? sig(n) + "|" + hint(n) : null),
    map,
    used,
  );
  [rb, rs] = rem();
  const similar = (a, b) => {
    // LCS ratio on text hints; empty-vs-empty counts as similar
    const x = hint(a),
      y = hint(b);
    if (!x && !y) return true;
    if (!x || !y) return false;
  ...
  for (const b of rb) {
    if (!isEl(b)) continue;
    const c = rs.filter(
      (s) => isEl(s) && !used.has(s) && sig(s) === sig(b) && similar(b, s),
    );
    if (!c.length) continue;
    const bi = bKids.indexOf(b);
    c.sort(
      (x, y) =>
        Math.abs(sKids.indexOf(x) - bi) - Math.abs(sKids.indexOf(y) - bi),
    );
    map.set(b, c[0]);
    used.add(c[0]);
  }
  ...
  let cursor = 0;
  for (const b of rb)
    for (let i = cursor; i < rs.length; i++) {
      const s = rs[i];
      if (
        used.has(s) ||
        b.nodeType !== s.nodeType ||
        (isEl(b) && s.tagName !== b.tagName)
      )
        continue;
...
function alignTree(bRoot, sRoot) {
  const map = new Map([[bRoot, sRoot]]),
    used = new Set([sRoot]);
  const walk = (b, s) => {
    alignKids([...b.childNodes], [...s.childNodes], map, used);
    for (const bk of b.childNodes)
      if (map.has(bk) && isEl(bk)) walk(bk, map.get(bk));
  };
  walk(bRoot, sRoot);
  ...
  const del2 = del.filter((e) => !map.has(e)),
    ins2 = ins.filter((e) => !used.has(e));
  for (const d of del2) {
    const c = ins2.filter(
      (s) => !used.has(s) && sig(s) === sig(d) && similarEl(d, s),
    );
    if (c.length === 1) {
      map.set(d, c[0]);
      used.add(c[0]);
...
function merge3(baseDoc, localDoc, remoteDoc) {
  ...
  const L = alignTree(baseDoc.body, localDoc.body),
    R = alignTree(remoteDoc.body ? baseDoc.body : baseDoc.body, remoteDoc.body);
  const conflicts = [];
  const changed = (b, s) =>
    !s || (isEl(b) ? b.outerHTML !== s.outerHTML : b.nodeValue !== s.nodeValue);
  const inParent = (side, b, parentSide) => {
    const s = side.map.get(b);
    return s && s.parentNode === parentSide ? s : null;
  };
  ...
  function mergeAttrs(b, l, r, el) {
    ...
    for (const n of names) {
      const bv = b.getAttribute(n),
        lv = l ? l.getAttribute(n) : bv,
        rv = r ? r.getAttribute(n) : bv;
      let v;
      if (lv === rv) v = lv;
      else if (lv === bv) v = rv;
      else if (rv === bv) v = lv;
  ...
  function mergeNode(b, l, r) {
    if (isText(b)) {
      const m = merge3Text(
        b.nodeValue,
        l ? l.nodeValue : b.nodeValue,
        r ? r.nodeValue : b.nodeValue,
      );
  ...
  function mergeChildren(b, l, r) {
    ...
    const lKids = l ? [...l.childNodes] : [],
      rKids = r ? [...r.childNodes] : [];
    // which side supplies the order of kept children? a side that reordered them
    const orderOf = (side, kids) =>
      kids
        .filter((k) => side.rev.has(k) && side.rev.get(k).parentNode === b)
        .map((k) => bKids.indexOf(side.rev.get(k)));
    const mono = (xs) => xs.every((x, i) => i === 0 || x >= xs[i - 1]);
    const lReordered = l && !mono(orderOf(L, lKids)),
      rReordered = r && !mono(orderOf(R, rKids));
    const O = rReordered
      ? { side: R, kids: rKids, other: L, otherKids: lKids }
      : lReordered
        ? { side: L, kids: lKids, other: R, otherKids: rKids }
        : { side: R, kids: rKids, other: L, otherKids: lKids };
    const result = [];
    const emitted = new Set();
    const emitBase = (bk) => {
      if (emitted.has(bk)) return;
      emitted.add(bk);
      const lk = L.map.get(bk),
        rk = R.map.get(bk);
      const lHere =
          (lk && lk.parentNode === (l || null)) || (lk && !l && false),
        rHere = rk && rk.parentNode === (r || null);
      const lMovedOut = lk && l && lk.parentNode !== l,
        rMovedOut = rk && r && rk.parentNode !== r;
      ...
      const merged = mergeNode(bk, lk || null, rk || null);
      merged.__bk = bk;
      result.push(merged);
    };
    const emitSideNode = (side, sk) => {
      ...
      if (bk) {
        ...
        emitted.add(bk);
        result.push(
          mergeNode(bk, L.map.get(bk) || null, R.map.get(bk) || null),
        );
        return;
      }
    ...
    const otherInsertsAfter = new Map();
    let anchor = null;
    for (const sk of O.otherKids) {
      const bk = O.other.rev.get(sk);
      if (bk && bk.parentNode === b) {
        anchor = bk;
        continue;
      }
      if (bk) {
        /* moved-in on other side */
      }
      if (!otherInsertsAfter.has(anchor)) otherInsertsAfter.set(anchor, []);
      otherInsertsAfter.get(anchor).push(sk);
    }
    for (const sk of otherInsertsAfter.get(null) || [])
      emitSideNode(O.other, sk);
    for (const sk of O.kids) {
      emitSideNode(O.side, sk);
      const bk = O.side.rev.get(sk);
      if (bk && otherInsertsAfter.has(bk))
        for (const x of otherInsertsAfter.get(bk)) emitSideNode(O.other, x);
    }
    ...
    for (let i = 0; i < bKids.length; i++) {
      ...
      let at = 0;
      for (let j = i - 1; j >= 0; j--) {
        const idx = result.findIndex((n) => n.__bk === bKids[j]);
        if (idx >= 0) {
          at = idx + 1;
          break;
        }
      ...
      node.__bk = bk;
      result.splice(at, 0, node);
    }
    return result;
  }
  for (const side of [L, R]) {
    side.rev = new Map();
    for (const [b, s] of side.map) side.rev.set(s, b);
  }
...
describe("PROTOTYPE three-way merge on the same scenarios", function () {
  it("runs", function () {
    ...
    run(
      "S10 (remote deletes untouched para, local edits another)",
