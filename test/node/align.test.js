...
function run(baseHtml, sideHtml, { ignore, idOf = defaultIdentity } = {}) {
  const b = parse(doc(baseHtml)).body,
    s = parse(doc(sideHtml)).body;
...
test("T-A1 identity pairs across parents", () => {
  const { b, s, a } = run(
    '<div><p data-id="x">hi</p></div><section></section>',
    '<div></div><section><p data-id="x">hi!</p></section>',
  );
...
test("T-A3 identical siblings keep order and are marked identical", () => {
  ...
  a.pairIdenticalChildren(q(b, "ul")); // identical subtrees pair their children on demand
  for (let i = 0; i < 3; i++)
    assert.equal(a.map.get(q(b, "li", i)), q(s, "li", i));
});

test("T-A4 a retyped heading pairs by signature and similar text", () => {
  const { b, s, a } = run(
    "<h2>Pricing plans for teams</h2><h2>About</h2>",
    "<h2>Pricing plans for everyone</h2><h2>About</h2>",
  );
