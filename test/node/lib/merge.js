...
export function mergeBodies(base, local, remote, opts = {}) {
  const b = parse(doc(base, opts.baseHead || "")), l = parse(doc(local, opts.localHead || "")), r = parse(doc(remote, opts.remoteHead || ""));
  const res = merge3(b, l, r, {
    identity: { base: opts.idOf || defaultIdentity, local: opts.idOf || defaultIdentity, remote: opts.idOf || defaultIdentity },
  ...
  return { html: res.doc.body.innerHTML, head: res.doc.head.innerHTML, res, b, l, r };
}
