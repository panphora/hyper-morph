...
import { createIdentityStore, importMap, indexByIdentity, defaultIdentity } from "../../src/identity.js";
...
test("T-P1..3 doctype sync", () => {
  ...
  const legacy = parse('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Strict//EN" "x"><html></html>');
...
test("T-I1 ignore predicate called at most once per element and is ancestor-aware", () => {
  const d = parse(doc('<div><section no-save><p><b>x</b></p></section><p>y</p></div>'));
  let calls = 0;
  const ignored = makeIgnore((el) => { calls++; return el.hasAttribute("no-save"); });
  const b = d.querySelector("b"), sec = d.querySelector("section"), py = d.querySelectorAll("p")[1];
  ...
  const before = calls;
  ignored(b); ignored(sec); ignored(py); ignored(d.querySelector("p"));
...
test("identity store mints, exports by path, imports by path, adopts", () => {
  ...
  const pairs = new Map();
  (function pair(l, c) { pairs.set(c, l); for (let i = 0; i < l.children.length; i++) pair(l.children[i], c.children[i]); })(live, clone);
  ...
  assert.equal(store.exportMap(clone, (n) => pairs.get(n) || null)[""], "tab1:1"); // stable
...
test("T-A2 duplicate ids on a side are dropped from the index; ignored subtrees skipped", () => {
  const d = parse(doc('<div id="a"></div><div id="a"></div><div id="b"></div><section no-save><div id="c"></div></section>'));
  const idx = indexByIdentity(d.body, defaultIdentity, makeIgnore((el) => el.hasAttribute("no-save")));
