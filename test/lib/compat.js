...
const HISTORY_IGNORE = '[editor-ui],[clay~="editor-ui"],[no-undo],[clay~="no-undo"]';
...
function baseFor(oldEl, mergeBase) {
  ...
  if (mergeBase.nodeType === 9) mergeBase = mergeBase.body.firstElementChild || mergeBase.documentElement;
...
export function morph(oldNode, newContent, config = {}) {
  ...
  const ignore = policy === "raw" ? undefined : (el) => el.matches(policy === "history" ? HISTORY_IGNORE : SYNC_IGNORE) || isExtensionNode(el);
  ...
  const options = {
    ...
    scripts: { execute: cfg.scripts?.handle !== false, merge: cfg.scripts?.merge !== false, mergeTags: cfg.scripts?.mergeTags || [] },
    head: { awaitLoads: !!cfg.head?.block, preserve: cfg.head?.shouldPreserve || (() => false) },
  };
  if (typeof cfg.key === "function") options.identity = { base: cfg.key, local: cfg.key, remote: cfg.key };

  if (oldNode && oldNode.nodeType === 9) {
    const remote = typeof newContent === "string" ? newContent : newContent.nodeType === 9 ? newContent : newContent.ownerDocument;
    return HM.mergeDocument(Object.assign({}, options, { live: oldNode, base: cfg.scripts?.mergeBase || null, remote }));
  }
  if (oldNode.tagName === "HTML" && oldNode.parentNode && oldNode.parentNode.nodeType === 9) {
    const remote = typeof newContent === "string" ? newContent : newContent.nodeType === 9 ? newContent : newContent.ownerDocument;
    const o = Object.assign({}, options, { live: oldNode.ownerDocument, base: cfg.scripts?.mergeBase || null, remote });
